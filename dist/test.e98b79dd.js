...
var _default = string; //把一堆东西放在一个新建的文件里面 然后export导出  再在需要用的文件里import导入  这样比较方便   也叫“模块化”
...
var player = {
  ...
  n: 1,
  init: function init() {
    player.ui.demo.innerText = _css.default.substr(0, player.n), player.ui.demo2.innerHTML = _css.default.substr(0, player.n), player.bindEvents(), player.play(); //现在这些函数变成属性了 要加player.
  },
  bindEvents: function bindEvents() {
    for (var key in player.events) {
      if (player.events.hasOwnProperty(key)) {
        var value = player.events[key]; //通过字符串去取对应的方法
